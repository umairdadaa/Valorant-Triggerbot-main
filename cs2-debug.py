# CS2 TriggerBot - Debug Version
import subprocess
import sys
import os

# ===== AUTO-INSTALL DEPENDENCIES =====
required_packages = ['pymem', 'keyboard', 'pynput', 'pywin32', 'requests']
...
def main():
    ...
    print("=" * 50)
    print("CS2 TriggerBot - DEBUG VERSION")
    print("=" * 50)
    ...
    while True:
        try:
            loop_count += 1
            if loop_count % 100 == 0:
                debug_print(f"Loop {loop_count} - Trigger Active: {trigger_active}")
            
            # Check if CS2 is in focus
            window_title = GetWindowText(GetForegroundWindow())
            if window_title != "Counter-Strike 2":
                if loop_count % 100 == 0:
                    debug_print(f"CS2 not in focus. Current window: '{window_title}'")
                time.sleep(0.1)
                continue

            if trigger_active:
                debug_print("Trigger is ACTIVE - attempting to read memory...")
                try:
                    # Read player
                    player = pm.read_longlong(client_base + dwLocalPlayerPawn)
                    debug_print(f"Player address: {hex(player)}")
                    
                    if player == 0:
                        debug_print("Player is 0 - not in game?")
                        time.sleep(0.05)
                        continue
                    
                    # Read entity ID
                    entityId = pm.read_int(player + m_iIDEntIndex)
                    debug_print(f"Entity ID: {entityId}")

                    if entityId > 0:
                        entList = pm.read_longlong(client_base + dwEntityList)
                        debug_print(f"Entity List: {hex(entList)}")
                        
                        entEntry = pm.read_longlong(entList + 0x8 * (entityId >> 9) + 0x10)
                        debug_print(f"Entity Entry: {hex(entEntry)}")
                        
                        entity = pm.read_longlong(entEntry + 120 * (entityId & 0x1FF))
                        debug_print(f"Entity: {hex(entity)}")

                        entityTeam = pm.read_int(entity + m_iTeamNum)
                        playerTeam = pm.read_int(player + m_iTeamNum)
                        debug_print(f"Player Team: {playerTeam}, Entity Team: {entityTeam}")

                        if entityTeam != playerTeam:
                            entityHp = pm.read_int(entity + m_iHealth)
                            debug_print(f"Entity HP: {entityHp}")
                            ...
                            if entityHp > 0:
                                ...
                                debug_print("Entity is dead (HP <= 0)")
                        else:
                            debug_print("Entity is on same team - not shooting")
                    else:
                        debug_print("No entity targeted (entityId <= 0)")
                        
                except Exception as e:
                    debug_print(f"Error in main loop: {e}")
            else:
                if loop_count % 100 == 0:
                    debug_print("Trigger is INACTIVE - waiting...")
                time.sleep(0.05)
            ...
            debug_print(f"Outer exception: {e}")
            time.sleep(0.1)
